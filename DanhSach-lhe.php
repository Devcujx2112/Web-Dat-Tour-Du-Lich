...
<li><a href="formlogin.php"><i class="fa-solid fa-user" style="color: white; margin-top:3px"></i></a></li>
  ...
  <div class="container" style="padding-top: 100px;">
    <h2 style="color:#2d4271; font-weight: bold;">Danh sách liên hệ</h2>
    <br>
  <table class="table">
  ...
  <?php 
    // chọn vào bảng db
    $lietke_sql = "SELECT * FROM tbl_lienhe order by LoaiThongTin, HoVaTen";
    //Thực thi câu lệnh
    ...
    ?>
      <tr>
      <td><?php 
        if($r['LoaiThongTin'] == "DuLich"){
          echo "Du Lịch";
        }if($r['LoaiThongTin'] == "ChamSocKhachHang"){
          echo "Chăm sóc khách hàng";
        }if($r['LoaiThongTin'] == "Khac"){
          echo "Khac";
        }
        ?>
        </td>       
      ...
      <td><a href="admin/Sua-lhe.php?sid=<?= $r['uid'];?>" class ="btn btn-warning">Sửa</a>      
        <a onclick="return confirm('Bạn có muốn xóa thông tin người này không?');" href="admin/Xoa-lhe.php?sid=<?=$r['uid'];?>" class="btn btn-danger">Xóa</a>
      </tr>
  <?php
    }
  ?>
