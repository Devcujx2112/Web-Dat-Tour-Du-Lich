...
<li><a href="formlogin.php"><i class="fa-solid fa-user" style="color: white; margin-top:3px"></i></a></li>
  ...
  <div class="container" style="padding-top: 100px;">
    <h2 style="color:#2d4271; font-weight: bold;">Danh sách tour du lịch</h2>
    <br>
  <table class="table">
  ...
  <?php 
    // chọn vào bảng db
    $lietke_sql = "SELECT * FROM tbl_dulich order by TieuDe, NoiKhoiHanh";
    //Thực thi câu lệnh
    ...
    ?>
      <tr>
        ...
        <td><img src="img/<?= $r['image'];?>" width="170px"></td>
        <td><?php echo $r['Time'];?></td>
        <td><?php echo $r['NoiKhoiHanh'];?></td>
        <td><?php echo $r['Gia'];?></td>
        <td><?php echo $r['mien'];?></td>
        <td><?php echo $r['MaTour'];?></td>
      
      <td><a href="admin/Sua-dlich.php?sid=<?= $r['uid'];?>" class ="btn btn-warning">Sửa</a>      
        <a onclick="return confirm('Bạn có muốn xóa thông tin tour này không?');" href="admin/Xoa-dlich.php?sid=<?=$r['uid'];?>" class="btn btn-danger">Xóa</a>
      </td>
      </tr>
  <?php
    }
  ?>
    ...
    <form action="admin/Them-dlich.php" method="post">
        <!-- Các trường và nút submit nếu cần -->
        <input type="submit" value="Thêm Tour" class="btn btn-primary">    
       </form>
      </div>
